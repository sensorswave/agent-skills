...
漏斗、留存优先用分析模型。它们经常要一次查出整体转化、分时间桶趋势和属性分组；`WINDOW_FUNNEL` / `RETENTION` 每次只能算一个维度。只需要单个维度下的转化或留存时，用这两个官方函数更简单，通常也更快，同一指标口径与分析模型基本一致。不要用 `GROUP BY DATE_TRUNC(time, 'day')` 套 `WINDOW_FUNNEL` 去模拟按日趋势：每个分组只能看到当天事件，转化窗口无法跨天。

事件分析改写成 `COUNT` / `COUNT(DISTINCT ssid)` 时，时区、筛选和用户属性快照对齐后，结果应对得上。
